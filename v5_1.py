...
class BankInterest(object):
    ...
    def diff_int(self):
        ...
        mes = []
        itog = 0
        osnd = self.summ/self.period
        prcinm = self.perc/1200
        mp_cnt = self.period
        rest = self.summ
        ...
        while mp_cnt != 0:
            ...
            mes.append(round(rest*prcinm, 2))
            rest = rest - mp_real
            mp_cnt = mp_cnt - 1
        return arr, round(itog, 2), ost, mes,osnd
    ...
    def ann_int(self):
        ...
        for i in range(mp_cnt):
            d = round(summ1*prcinm,2)
            osn = mp - d
            mes.append(d)
            summ1 = summ1 - osn
        return round(mp, 2), round(total, 2),mes
...
class Ui_MainWindow(object):
        def setupUi(self, MainWindow):
                ...
                self.summ.textEdited.connect(self.otdelnulperv)
                ...
                self.sumnedred.textEdited.connect(self.otdelnulipoteca)
        ...
        def retranslateUi(self, MainWindow):
                ...
                MainWindow.setWindowTitle(_translate("MainWindow", "Кредитный ипотечный калькулятор"))
                ...
                self.label.setText(_translate("MainWindow", "Кредитный ипотечный калькулятор"))
        ...
        def otdelnulipoteca(self):
                text = str(self.sumnedred.text())
                text = text.replace(' ', '')
                dlin = len(text)
                self.sumnedred.setText(self.probel(text, dlin))

        def otdelnulperv(self):
                text = str(self.summ.text())
                text = text.replace(' ', '')
                dlin = len(text)
                self.summ.setText(self.probel(text, dlin))

        def probel (self,text,dlin):
            itog = ''
            while dlin - 3 > 0:
                itog = text[dlin - 3:dlin] + ' ' + itog
                dlin -= 3
            itog = text[0:dlin] + ' ' + itog
            itog = itog.strip(" ")
            return str(itog)
        ...
        def checker_perv(self, s):
                k = 0
                textperv = self.summ.text()
                textperv = textperv.replace(' ', '')
                try:
                    perv = int(textperv)
                except ValueError:
                    perv = -1
        ...
        def checker_ipoteca(self, s, perv):
                k = 0
                textipoteca = self.sumnedred.text()
                textipoteca = textipoteca.replace(' ', '')
                try:
                    ipoteca = int(textipoteca)
                except ValueError:
                    ipoteca = -1
                    k += 1
                try:
        ...
        def diff_table(self, srok, arr, ost, mes, osnd):
                ...
                for i in range(srok):
                    self.table.setItem(i, 1, QTableWidgetItem(str(round(arr[i], 1))))
                ost[srok-1]  = 0                  
                for i in range(srok):
                    self.table.setItem(i, 4, QTableWidgetItem(str(round(ost[i], 1))))
                for i in range(srok):
                    self.table.setItem(i, 3, QTableWidgetItem(str(round(mes[i], 1))))
                    self.table.setItem(i, 2, QTableWidgetItem(str(round(osnd, 1))))

        def annuit_table(self, srok, mp, total, mes):
                currentMonth = datetime.now().month + 1
                currentYear = datetime.now().year
                list_calendar = ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь",
                                 "Ноябрь", "Декабрь"]
                ...
                for i in range(srok):
                    ...
                    self.table.setItem(i, 0, QTableWidgetItem(data))
                for i in range(srok):
                    self.table.setItem(i, 1, QTableWidgetItem(str(round(mp, 1))))
                ost = total
                for i in range(srok):
                    if i != srok-1:
                        ost = ost - mp
                ...
                for i in range(srok):
                    self.table.setItem(i, 3, QTableWidgetItem(str(round(mes[i], 1))))
                    self.table.setItem(i, 2, QTableWidgetItem(str(round(mp - mes[i], 1))))
                ...
                try:
                    if (perv != -1 and procenty != -1 and srok != -1 and ipoteca != -1 and plat != -1):
                        ...
                        if self.diff.isChecked():
                            arr, total, ost, mes, osnd = a.diff_int()
                            ...
                            self.diff_table(srok, arr, ost, mes, osnd)

                        if self.annuit.isChecked():
                            mp, total, mes = a.ann_int()
                            self.sinm.setText("Сумма в месяц")
                            self.sinmred.setText(str(mp) + ' р')
                            self.vsegred.setText(str(total) + ' р')
                            perepl = total - (ipoteca - perv)
                            self.pereplata.setText(str(round(perepl, 2)) + ' р')
                            self.annuit_table(srok, mp, total, mes)
                    else:
                        self.oshibka(s)
                except:
                    pass
